...
select CONCAT(employee_id, '-', FIRST_NAME, ' ', Last_name) as new_name 
from Employees
...
----3.	Write a query that displays the first name and the length of the first name for all employees whose name starts with the letters 'A', 'J' or 'M'. 
...
SELECT *
FROM SingleOrder
ORDER BY CASE WHEN id = 0 THEN 999999999999 ELSE id END;
...
select *, CONCAT(SequenceNumber, ' ', String) as new_name 
from DMLTable
...
SELECT CONCAT(FIRST_NAME, ' ', LAST_NAME) AS Full_name,
       LEN(CONCAT(FIRST_NAME, LAST_NAME)) - LEN(REPLACE(CONCAT(FIRST_NAME, LAST_NAME), 'a', '')) AS count_a
FROM Employees
WHERE (LEN(CONCAT(FIRST_NAME, LAST_NAME)) - LEN(REPLACE(CONCAT(FIRST_NAME, LAST_NAME), 'a', ''))) >= 3;

---------------------------------------------------------

select concat(FIRST_NAME,' ',LAST_NAME) as full_name from Employees_
where patindex('%a%a%a%',concat(FIRST_NAME,' ',LAST_NAME))<>0

select concat(FIRST_NAME,' ',LAST_NAME) as full_name, replace(concat(FIRST_NAME,' ',LAST_NAME),'a','') from Employees_
where len(concat(FIRST_NAME,' ',LAST_NAME))-len(replace(concat(FIRST_NAME,' ',LAST_NAME),'a',''))>=3

-----------------------------------------------------------------

select 'aliasdygasyudgalisadhgsajhgali',REPLACE('aliasdygasyudgalisadhgsajhgali','ali',''),
(len('aliasdygasyudgalisadhgsajhgali')-len(REPLACE('aliasdygasyudgalisadhgsajhgali','ali','')))/3
...
SELECT 
    ...
    CAST(SUM(CASE WHEN DATEDIFF(year, hire_date, GETDATE()) > 3 THEN 1 ELSE 0 END) AS FLOAT) 
      / COUNT(employee_id) * 100 AS percentage_over_3_years
FROM Employees
...
SELECT 
    JobDescription,
    MAX(missionCount) AS MaxMissions,
    MIN(missionCount) AS MinMissions
FROM Personal
...
FROM Student S
JOIN (
    SELECT MONTH(Birthday) AS BMonth, DAY(Birthday) AS BDay
    FROM Student
    GROUP BY MONTH(Birthday), DAY(Birthday)
    HAVING COUNT(*) > 1
) AS B
ON MONTH(S.Birthday) = B.BMonth AND DAY(S.Birthday) = B.BDay;
