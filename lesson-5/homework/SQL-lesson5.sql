...
(40, 'Compact Dishwasher', 470.00, 'Electronics', 18);
	...
	--Команда SELECT DISTINCT в SQL используется для удаления дубликатов из результатов запроса. 
	То есть она возвращает только уникальные строки.--


	SELECT DISTINCT LastName, Country FROM Customers;


    6. Write a query that uses CASE to create a conditional column 
...
Medium-Level Tasks
	...
	--Оператор EXCEPT в SQL используется для получения разности множеств — то есть, 
	--он возвращает строки из первого запроса, которых нет во втором.
	...
	Select employeeID, Age, Salary from Employees
