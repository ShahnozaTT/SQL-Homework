...
Easy-Level Tasks
	...
	select Category, Sum(StockQuantity) as total_number_of_products
	from Products
	group by Category

    --2. Using Products table, get the average price of products in the 'Electronics' category.

	select	 avg(Price) as avg_price__of_products
	from Products
	...
	select Country, count(customerID) as total_number_of_customers
	from Customers
	group by Country
	...
	select * from Orders
	Select min(Quantity) as min_qty_ordered, max(quantity) as max_qty_ordered
	from Orders
...
Medium-Level Tasks
	...
	,avg(quantity) as average_order_amount from orders
...
SUM(Population) for year in([2012],[2013])
	...
	PIVOT (	SUM(Population) for district_name in ([Bektemir], [Chilonzor], [Yakkasaroy])) as Population_Each_City
