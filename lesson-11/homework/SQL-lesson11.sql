create database SQLHomework11_1
use SQLHomework11_1
go
...
(40, 10, 1, '2024-07-07', 600.00);
	...
	Select Orders.OrderID, Customers.FirstName+' '+Customers.LastName as CustomerName, Orders.OrderDate
	from Orders
	...
	Select Customers.FirstName+' '+Customers.LastName as CustomerName, Orders.OrderID, Orders.OrderDate
	from Customers
	inner join Orders
	...
	select Customers.FirstName+' '+Customers.LastName as CustomerName, count(Orders.OrderID) as TotalOrders
	from Orders
	left join Customers
	on Orders.CustomerID=Customers.CustomerID
	...
	select Customers.FirstName+' '+Customers.LastName as CustomerName, max(Orders.OrderDate)  as MostRecentOrderDate
	from Customers
	left join Orders
	on Customers.CustomerID=Orders.CustomerID
	...
	Select * from Customers
		...
		select Customers.FirstName+' '+Customers.LastName as CustomerName, SUM(Orders.TotalAmount) as OrderTotal
		from Orders
		inner join Customers
		on Orders.CustomerID=Customers.customerID
		group by Customers.FirstName+' '+Customers.LastName
...
inner join Departments
	...
	select Employees.Name as EmployeeName, Departments.DepartmentName, Employees.HireDate
	from Employees
	inner join Departments
	on Employees.DepartmentID=Departments.DepartmentID
	...
	select Customers.FirstName+' '+Customers.LastName as CustomerName,Orders.OrderID, Customers.Address, Orders.OrderDate
	from Customers
	left join Orders
	on Customers.CustomerID=Orders.CustomerID 
	...
	Select Categories.CategoryName, count(distinct Products.ProductName) as ProductCount
	from Products
	...
	where Departments.DepartmentName in ('Human Resources','Finance') or (
        LEN(Employees.Name) 
        - LEN(REPLACE(LOWER(Employees.Name), 'a', ''))
        - LEN(REPLACE(LOWER(Employees.Name), 'e', ''))
        - LEN(REPLACE(LOWER(Employees.Name), 'i', ''))
        - LEN(REPLACE(LOWER(Employees.Name), 'o', ''))
        - LEN(REPLACE(LOWER(Employees.Name), 'u', '')) >= 4
    );
