...
Easy-Level Tasks (10)
	...
	count(employeeId) as Num__of_emp
	from Employees
	group by DepartmentName
	...
	select count(EmployeeID) as Num_of_Emp
	from Employees
	...
	Select departmentName, count(employeeId) as total_emp, AVG(Salary) as avg_salary
	from Employees
	group by departmentName
	...
	select * from Sales
	select year(SaleDate) as YearOnly, sum(SaleAmount) as total_sales
	from Sales
 ...
 select * from Employees
 select DepartmentName, avg(salary) as avg_salary_expenses
 from employees
 group by DepartmentName
...
SELECT 
  CustomerID,
  SUM(CASE WHEN totalAmount > 50 THEN totalAmount ELSE 0 END) AS total_amount_over_50,
...
GROUP BY CustomerID;
 ...
 25. Write a query to find the MIN and MAX order quantity per Year. From orders table.
  ...
  select * from Orders
  select Year(OrderDate), min(quantity) as min_qty, max(quantity) as max_qty
  from orders
